...
logging_with_default_config_works(Config) ->
    ...
    MainFileHandler = get_handler_by_id(Handlers, rabbitmq_log_file_1),
    ...
    UpgradeFileHandler = get_handler_by_id(Handlers, rabbitmq_log_file_2),
    ...
    ?assert(ping_log(rabbitmq_log_file_1, info)),
    ?assert(ping_log(rabbitmq_log_file_1, info,
                     #{domain => ?RMQLOG_DOMAIN_GLOBAL})),
    ?assert(ping_log(rabbitmq_log_file_1, info,
                     #{domain => ['3rd_party']})),
    ?assertNot(ping_log(rabbitmq_log_file_1, info,
                        #{domain => ?RMQLOG_DOMAIN_UPGRADE})),

    ?assert(ping_log(rabbitmq_log_file_2, info,
                     #{domain => ?RMQLOG_DOMAIN_UPGRADE})),
    ?assertNot(ping_log(rabbitmq_log_file_2, info,
                        #{domain => ?RMQLOG_DOMAIN_GLOBAL})),
    ok.
...
logging_to_stdout_configured_in_env_works(Config) ->
    ...
    logging_to_stddev_works(standard_io, stdout, Config, Context).
...
logging_to_stderr_configured_in_env_works(Config) ->
    ...
    Context = Context0#{main_log_file => "-stderr",
                        var_origins => Origins0#{
                                         main_log_file => environment}},
    logging_to_stddev_works(standard_error, stderr, Config, Context).
...
logging_to_stddev_works(Stddev, Id, Config, Context) ->
    ...
    UpgradeFileHandler = get_handler_by_id(Handlers, rabbitmq_log_file_1),
    UpgradeFile = upgrade_log_file_in_context(Context),
    ?assertNotEqual(undefined, UpgradeFileHandler),
    ?assertMatch(
       #{level := info,
...
logging_to_exchange_works(Config) ->
    ...
    ExchangeHandler = get_handler_by_id(Handlers, exchange),
    ...
    ?assert(ping_log(exchange, info, Config1)),
    ?assert(ping_log(exchange, info,
                     #{domain => ?RMQLOG_DOMAIN_GLOBAL}, Config1)),
    ?assert(ping_log(exchange, info,
                     #{domain => ['3rd_party']}, Config1)),
    ?assertNot(ping_log(exchange, info,
...
setting_log_levels_in_env_works(Config) ->
    ...
    ?assertNot(ping_log(rabbitmq_log_file_1, GlobalLevel,
                        #{domain => ?RMQLOG_DOMAIN_PRELAUNCH})),
    ?assertNot(ping_log(rabbitmq_log_file_1, info,
                        #{domain => ['3rd_party']})),
    ?assertNot(ping_log(rabbitmq_log_file_1, info,
    ...
    ?assert(ping_log(rabbitmq_log_file_1, GlobalLevel)),
    ?assert(ping_log(rabbitmq_log_file_1, GlobalLevel,
                     #{domain => ?RMQLOG_DOMAIN_GLOBAL})),
    ?assert(ping_log(rabbitmq_log_file_1, PrelaunchLevel,
                     #{domain => ?RMQLOG_DOMAIN_PRELAUNCH})),
    ?assert(ping_log(rabbitmq_log_file_1, GlobalLevel,
                     #{domain => ['3rd_party']})),
    ?assertNot(ping_log(rabbitmq_log_file_1, GlobalLevel,
    ...
    ?assert(ping_log(rabbitmq_log_file_2, GlobalLevel,
                     #{domain => ?RMQLOG_DOMAIN_UPGRADE})),
    ?assertNot(ping_log(rabbitmq_log_file_2, GlobalLevel,
                        #{domain => ?RMQLOG_DOMAIN_GLOBAL})),
    ok.
